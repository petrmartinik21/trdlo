<template>
  <form @submit.prevent="formSubmit" id="form" class="bg-sky-800">
    ...
    <button
      ...
      id="submit-button"
      :disabled="!isValid"
      class="border-double border-2 border-stone-600 bg-stone-500 hover:cursor-pointer hover:bg-stone-400 hover:border-stone-800 hover:border-double hover:border-2 rounded-full px-4 m-2"
    >
      <slot name="icon"></slot>
    </button>
  </form>
